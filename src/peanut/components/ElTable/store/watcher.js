...
export default Vue.extend({
    ...
    methods: {
        ...
        updateFilters(columns, values) {
            ...
            const states = this.states;
            columns.forEach(col => {
                states.filters[col.id] = {
                    key: col.columnKey || col.prop || col.id,
                    values,
                    type: col.filterType
                };
            });

            return Object.keys(states.filters).map(id => ({
                key: states.filters[id].key,
                values: [...states.filters[id].values],
                type: states.filters[id].type
            }));
        },
        ...
        execFilter() {
            ...
            Object.keys(filters).forEach((columnId) => {
                const values = states.filters[columnId].values;
            ...
            if (Array.isArray(columnKeys)) {
                ...
                keys.forEach(key => {
                    ...
                    if (column) {
                        ...
                        panels[key].cachedFilters = [];
                        panels[key].lastFilters = null;
                    }
                });
